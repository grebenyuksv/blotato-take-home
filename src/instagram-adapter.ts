...
/**
 * Instagram: POST /{ig_comment_id}/replies
 * IG attaches replies to the top-level comment by documented design when the
 * target is itself a reply; we still pass the id we were given.
 */
export function createInstagramAdapter(
  ...
  return {
    async publishReply(input: PublishReplyInput): Promise<PublishReplyResult> {
      try {
        const res = await http.post(`/${input.platformCommentId}/replies`, {
          message: input.text,
